...
begin
 ...
 if v_upper in ('STOP','STOPALL','UNSUBSCRIBE','CANCEL','END','QUIT','REVOKE') then
   if exists(select 1 from public.wf_domain_events where event_key='sms_retention_opt_out:'||v_provider_id) then
     return jsonb_build_object('kind','opt_out','status','opted_out','idempotent',true);
   end if;
   ...
   return jsonb_build_object('kind','opt_out','status','opted_out','idempotent',false);
 end if;
