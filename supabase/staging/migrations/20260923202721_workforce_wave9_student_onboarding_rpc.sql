create or replace function public.save_student_onboarding_step(
...
begin
  ...
  select o.profile_data into v_existing
  ...
  insert into public.wf_onboarding_accounts(
    ...
    v_auth_user_id,v_user_id,'student','in_progress',
    greatest(1,least(6,coalesce(p_current_step,1))),v_merged,now()
  )
  on conflict (auth_user_id) do update
  set selected_role='student',
      status=case when public.wf_onboarding_accounts.status='complete'
...
begin
  ...
  if not exists(
    select 1 from public.wf_role_memberships r
    where r.user_id=v_user_id and lower(r.role)='student' and lower(r.status)='active'
  ) then
    insert into public.wf_role_memberships(
      user_id,role,status,created_at,updated_at,bridge_source_key,bridge_source_sheet
    ) values(
      v_user_id,'student','active',v_now,v_now,
      'txkpro_workforce_native:role:'||v_user_id||':student',
      'txkpro_workforce_native'
    );
  end if;
...
revoke all on function public.save_student_onboarding_step(integer,jsonb) from public,anon;
revoke all on function public.complete_student_onboarding(jsonb) from public,anon;
grant execute on function public.save_student_onboarding_step(integer,jsonb) to authenticated,service_role;
